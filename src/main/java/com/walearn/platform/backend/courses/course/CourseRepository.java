...
public interface CourseRepository extends JpaRepository<Course, Integer>, CourseRepositoryCustom {
	 ...
	 * @throws IllegalArgumentException if {@literal uuid} is {@literal null}.
	 */
	@Query(value = "SELECT new com.walearn.platform.backend.courses.course.CourseLightDTO(c.uuid, c.name, c.description, c.numberOfStudents.number) " +
			"FROM com.walearn.platform.backend.courses.course.Course c")
	 ...
	 */
	@Query("select count(c) > 0 from Course c join com.walearn.platform.backend.courses.teacher.Teacher r on c.teacher = r.id where c.uuid = :uuid and r.username = :username")
